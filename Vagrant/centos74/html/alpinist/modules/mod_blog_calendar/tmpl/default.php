...
<?php if(isset($dropdown)) { ?>
	...
	<li class="droplist">
		...
		<?php } ?>
		<a href="<?php echo JRoute::_('index.php?option=com_blog_calendar'.'&year='.$listYear.'&modid='.$module->id.$ComBlogItemID); ?>">
		<?php echo $listYear ?>
		</a>
	...
	<a id="toggle-<?php echo $listYear . "-" . $month . "-" . $module->id ?>" onclick="toggle_visibility('<?php echo $listYear . "-" . $month . "-" . $module->id ?>')" href="javascript:void(0)"  >
					<?php if(isset($monthCount['now']) && $monthCount['now'] && $yearNow==$listYear && $displaySublevels && !($showYears && $showMonths)){ 
					echo "&#9660;"; }
							else{ echo "&#9658;"; } ?>
				</a>
	                <?php } ?>
			<a href="<?php echo JRoute::_('index.php?option=com_blog_calendar'.'&year='.$listYear.'&month='.$month.'&modid='.$module->id.$ComBlogItemID); ?>">
			<?php echo JText::_(date('F',mktime(12,30,30,$month,15,2000,0))) . ($showYears? '': ' '.$listYear)  ?>
		</a>
	...
	<?php if($showArticles){ ?>	
		<ul class="articles" style="display: <?php if(array_key_exists('now',$monthCount) && $monthCount['now'] && $yearNow==$listYear && 
