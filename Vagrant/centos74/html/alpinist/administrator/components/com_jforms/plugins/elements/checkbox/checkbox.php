...
*
* @version		$Id: checkbox.php BsAlpinist 2.5.0 $
* @package		Joomla
 ...
 * @subpackage JForms.Plugins
 */
class JFormEPluginCheckbox extends JFormEPlugin{
...
//		$inputStyle = $p->useCss?'':"width:{$p->cw}px;height:{$p->ch}px;";
		$inputStyle = $p->useCss?'':"width:{$p->cw}px;";
		
		$output .= _line("<div class='error-message' id='{$htmlId}_error'>$error</div>",2	);
		$output .= _line("<fieldset class='$inputClass' id='$htmlId' style='$inputStyle' data-role='controlgroup'>",2);
		...
		for($i=0;$i<count($elements);$i++){
			...
			if( $p->layout == 'List' ){
				$output .= _line('<div class="checkbox-vertical" style="display:block;" >',2);
			} else {
				$output .= _line('<div class="checkbox-horizontal" style="display:inline-block;" >',2);
			}
	...
	function jsValidation( $elementData ){
		...
		$output .= _line("{$p->hash}_error.innerHTML='".JText::_('FIELD_REQUIRED')."';",3);	
		$output .= _line("}" ,2);
		return $output;		
