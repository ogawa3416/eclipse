...
var email = new Class({
	...
	vaildate : function(){
		...
		if(!this.options.uinput && additionalEmails.length == 0){
			errors.push( "<?php echo JText::_('WITH_USER_INPUT_DISABLED_YOU_MUST_SPECIFY_AT_LEAST_ONE_ADDITIONAL_EMAIL'); ?>" );
		}
		...
		if(additionalEmails.length > 0){
			for(j=0;j<additionalEmails.length;j++){
				if( !this._validateEmail(additionalEmails[j]) && !this._validatePlaceholder(additionalEmails[j])){
					errors.push( "<?php echo JText::_('YOU_HAVE_AT_LEAST_ONE_INVALID_EMAIL_IN_ADDITIONAL_EMAILS_FIELD'); ?>" );
				}
			}
		}
		
		if(this.options.subject.trim().length == 0){
				errors.push( "<?php echo JText::_('EMAIL_SUBJECT_IS_EMPTY'); ?>" );
		}

		if(this.options.message.trim().length == 0){
				errors.push( "<?php echo JText::_('EMAIL_BODY_IS_EMPTY'); ?>" );
		}
