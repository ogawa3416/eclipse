...
* Mail storage  plugin
*
* @version		$Id: Mail.php BsAlpinist 2.5.0 $
* @package		Joomla
			...
			if( $field->type == 'file' && $attach ){
				foreach( $messages as $messageKey => $message ){
					if( preg_match( "/$fieldPlaceholder/i", $messages[$messageKey] )){
							...
							$mailer->AddAttachment(JFORMS_FS_PATH.DIRECTORY_SEPARATOR.$d->path);
					}
...
// BugFix. 2012-11-13 >>>>
		if( !$userEmail ) {
			...
			if( $myuser->id ) $userEmail = $myuser->email;
		}
// BugFix. 2012-11-13 <<<<
		if( $pluginSettings['SendAdmin'] ){
			...
			$mail->SetSubject(JText::_('NEW_ENTRY_ADDED'));
		...
		if( $pluginSettings['SendUser'] ){
			...
			if( JMailHelper::isEmailAddress( $userEmail ) ){
				...
				//$mail->setSender( array( $email, $name ) );
				$mail->SetSubject(JText::_('YOUR_ENTRY_HAS_BEEN_RECEIVED'));
