...
*
* @version		$Id: file.php BsAlpinist 2.5.0 $
* @package		Joomla
...
class JFormEPluginFile extends JFormEPlugin{
		...
		switch( $format ){
			...
			case 'raw':
				$linkText = $obj->name. '( '. $obj->size .' ' .JText::_('Bytes') .' )';
				$output = '';
				$output .= JText::_('FILE_URL')   .':'. $obj->link."\n";
				$output .= JText::_('FILE_NAME')  .':'. $obj->name."\n";
				$output .= JText::_('FILE_SIZE')  .':'. $obj->size."\n";
				return $output;
...
//		$labelStyle = $p->useCss?'':"width:{$p->lw}px;height:{$p->lh}px;";
		$labelStyle = $p->useCss?'':"width:{$p->lw}px;";
	...
	function jsValidation( $elementData ){
		...
		$output .= _line("{$p->hash}.className ='input-error $p->css';",4);
		$output .= _line("{$p->hash}_label.className ='label-error $p->css';",4);
		$output .= _line("{$p->hash}_error.innerHTML='".JText::_('UNSUPPORTED_EXTENSION')."';",4);	
		if( !empty( $p->extensions ) ){ 
			...
			$output .= _line("{$p->hash}_error.innerHTML='".JText::_('UNSUPPORTED_EXTENSION')."';",5);	
	...
	function validate( $elementData, $input ){
		...
		if( !array_key_exists( $p->hash, $_FILES ) || empty( $_FILES[$p->hash]['name'] ) ){
			...
			if( in_array( JFormEPluginFile::_getExtension($file['name']),explode(',', strtolower($p->veryBadExtensions) ))){
				...
				return JText::_('INVALID_FILE_EXTENSION');		
			}	
	...
	function beforeSave( $elementData, $input, $fsInfo = null ){
		...
		if( $file['error'] ){
			JError::raiseError( 500, JTEXT::_('ERROR_OCCURED_WHILE_UPLOADING_FILE_ERROR_CODE') .$file['error']);						
		}
		
		$fsPath = $fsInfo['path'];
		$fsUrl  = $fsInfo['url'];
	
		if( !JFile::upload( $file['tmp_name'], JFORMS_FS_PATH.DIRECTORY_SEPARATOR.$fsPath.DIRECTORY_SEPARATOR.$shortName )){
			JError::raiseError( 500, JTEXT::_('ERROR_OCCURED_WHILE_MOVING_UPLOADED_FILE'));							
		}
		$dbData = new stdClass();
		
		$dbData->path = $fsPath.DIRECTORY_SEPARATOR.$shortName;
		$dbData->link = $fsUrl .'/'.$shortName;
		$dbData->name = $file['name'];
		$dbData->size = filesize( JFORMS_FS_PATH.DIRECTORY_SEPARATOR.$fsPath.DIRECTORY_SEPARATOR.$shortName );
