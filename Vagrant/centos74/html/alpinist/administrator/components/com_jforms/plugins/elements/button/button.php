...
//		$style  = $useCss?'':"width:{$p->cw}px;height:{$p->ch}px;";
		$style  = $useCss?'':"width:{$p->cw}px;";
		$p->label = htmlspecialchars($p->label, ENT_QUOTES);
		
		switch( $p->func ){
			
			case 'Submit':
				$output .= _line("<input name='$p->hash' class='$css' type='button' onclick='Joomla.submitform(\"submit\",this.form)' value='$p->label' style='$style' data-inline='true'/>",2);
				break;
