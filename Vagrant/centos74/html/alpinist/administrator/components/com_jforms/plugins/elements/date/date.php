...
* Date Element plugin
*
* @version		$Id: date.php BsAlpinist 2.5.0 $
* @package		Joomla
...
class JFormEPluginDate extends JFormEPlugin {
	...
	function render( $elementData ){
		...
		if( !is_array( $default )){
			...
			if( $default[0] == '9999' ) {
				$default[0] = $now->format("Y",true);
				$default[1] = $now->format("m",true);
				$default[2] = $now->format("d",true);
			}
...
//		$labelStyle = $p->useCss?'':"width:{$p->lw}px;height:{$p->lh}px";
		...
		switch( $p->span ){
			...
			case -1:
				$endYear = intval($startYear) + $p->ospan;
				break;
				
			default:
				$endYear = intval($startYear) + $p->span;
				break;
		}
...
// for Bsalpinist
		...
		$output .= _line("<fieldset class='$inputClass' id='{$htmlId}_field' data-role='controlgroup' data-type='horizontal'>",2);
...
//								   $default[0], '', $htmlId.'_d' ), 2);
								   $default[0], '%d'.JText::_('BSW_DAYFORM'), $htmlId.'_d' ), 2);
			}  
			if( $e == 'MM' ){
...
//								  $default[2], '', $htmlId.'_y' ), 2);
								  $default[2], '%d'.JText::_('BSW_YEARFORM'), $htmlId.'_y' ), 2);
			}
		}
...
//			$arr[] = JHTML::_('select.option',  $fi, $fi );
			$arr[] = JHTML::_('select.option',  $i, $fi );
		}
