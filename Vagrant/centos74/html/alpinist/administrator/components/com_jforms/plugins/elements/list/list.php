...
*
* @version		$Id: list.php BsAlpinist 2.5.0 $
* @package		Joomla
...
class JFormEPluginList extends JFormEPlugin{
	...
	function beforeSave( $elementData, $input,$fsInfo=null ){return $input?implode(',',$input):'';}
...
//		$labelStyle = $p->useCss?'':"width:{$p->lw}px;height:{$p->lh}px;";
		...
		$output .= _line("<select data-native-menu='false' class='$inputClass' $multi name='".$p->hash."[]' id='$htmlId' style='$inputStyle' >",2);
	...
	function jsValidation( $elementData ){
		...
		$output .= _line("{$p->hash}_error.innerHTML='".JText::_('FIELD_REQUIRED')."';",5);	
		$output .= _line("}" ,4);
		$output .= _line("} else {" ,3);
