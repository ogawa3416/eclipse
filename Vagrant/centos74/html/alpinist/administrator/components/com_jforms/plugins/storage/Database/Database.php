...
*
* @version		$Id: Database.php BsAlpinist2.5.0 $
* @package		Joomla
...
class JFormSPluginDatabase extends JFormSPlugin{
	...
	function getFSPath( $form, $fieldName, $recordId, $create=true ){
		...
		return 
		array(	
			'path' => $tableName.DIRECTORY_SEPARATOR.$fieldName.DIRECTORY_SEPARATOR.$recordId,
	...
	function _createEmptyHtmlFile( $path ){
		if( !JFile::exists( $path.DIRECTORY_SEPARATOR.'index.html' ) ){
			...
			JFile::write( $path.DIRECTORY_SEPARATOR.'index.html',$textbuf );
		}
	}
	...
	function _createFSField( $tableName, $fieldName ){
		...
		if( !JFolder::exists( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName ) ){
			 JFolder::create( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName );
			 JFormSPluginDatabase::_createEmptyHtmlFile( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName );
		}
	}
	
	function _deleteFSField( $tableName, $fieldName ){
		if( JFolder::exists( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName ) ){
			JFolder::delete( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName );
		}
	}
	...
	function _createFSRecord( $tableName, $fieldName, $recordId ){
		...
		if( !JFolder::exists( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName.DIRECTORY_SEPARATOR.$recordId ) ){
			 JFolder::create( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName.DIRECTORY_SEPARATOR.$recordId );
 			 JFormSPluginDatabase::_createEmptyHtmlFile( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$fieldName.DIRECTORY_SEPARATOR.$recordId  );
	...
	function _deleteFSRecords( $tableName, $ids ){
		...
		if(!is_dir(JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR)) {
			return;
		}
		$fields = JFolder::folders( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR );
		...
		foreach( $fields as $field ){
			$records = JFolder::folders( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$field.DIRECTORY_SEPARATOR );
		 	foreach( $records as $record ){	
				if( $isDeleteAll || in_array( $record, $ids ) ){
					if( JFolder::exists( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$field.DIRECTORY_SEPARATOR.$record ) ){
						JFolder::delete( JFORMS_FS_PATH.$tableName.DIRECTORY_SEPARATOR.$field.DIRECTORY_SEPARATOR.$record );
					}
				}
			}
		}
	 ...
	 */
	public function getRecords( $form, $ids ){
	...
	function saveRecord( $form, $data ){
		...
		foreach( $form->fields as $f ){
			...
			//Clean-up content
			$valueArray[] = $db->quote( $db->escape( $data[$fname], false ), false );
		}
