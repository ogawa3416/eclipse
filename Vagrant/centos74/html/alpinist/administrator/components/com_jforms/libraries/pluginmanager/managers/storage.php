...
*
* @version		$Id: storage.php BsAlpinnist2.5.0 $
* @package		Joomla
...
require_once JFORMS_BACKEND_PATH.DIRECTORY_SEPARATOR.'models'.DIRECTORY_SEPARATOR.'JFormSPlugin.php';
...
//			$response[$p->name] = call_user_func_array(array($className,$name),$params);
			...
			$obj = new $className();
			$response[$p->name] = call_user_func_array(array($obj,$name),$params);
		}
		return $response;
	}
	...
	{
		$pluginCorePath  = JFORMS_BACKEND_PATH.DIRECTORY_SEPARATOR.'plugins'.DIRECTORY_SEPARATOR.'storage'.DIRECTORY_SEPARATOR.$name.DIRECTORY_SEPARATOR;
		$pluginMediaPath = JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'com_jforms'.DIRECTORY_SEPARATOR.'plugins'.DIRECTORY_SEPARATOR.'storage'.DIRECTORY_SEPARATOR.$name.DIRECTORY_SEPARATOR;
...
//		$root = JFactory::getXML($filename,true);
	...
	{
		$plugins = file(JFORMS_BACKEND_PATH.DIRECTORY_SEPARATOR."plugins".DIRECTORY_SEPARATOR."storage".DIRECTORY_SEPARATOR."plugins.list");
