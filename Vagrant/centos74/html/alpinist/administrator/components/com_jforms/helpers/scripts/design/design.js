...
*
* @version		$Id: design.js 2013-12-19 00:00: $
* @package		Joomla
* @subpackage	JForms
...
var correctionPixels = window.navigator.userAgent.toLowerCase().indexOf("msie") != -1?0:0;
...
function invokeMethod( plugin, method, parameters) {
	...
	var jtoken      = '<?php echo JSession::getFormToken(); ?>';
...
Joomla.submitbutton = function(pressbutton)
{
	...
	if( !hasObligatoryElement ){
		alert("<?php echo JText::_("YOU_MUST_HAVE_ATLEAST_ONE_ELEMENT_THAT_ALLOWS_USER_INPUT");?>");
		enableTab('toolbar');
		return false;
	}
	...
	if( !hasSubmitButton ){
/**************** BsAlpnist
		alert("<?php echo JText::_("YOU_MUST_HAVE_ATLEAST_ONE_SUBMIT_BUTTON_IN_THE_FORM");?>");
		enableTab('toolbar');
		return false;
****************/
	}
	...
	if( form.elements['params[title]'].value.length == 0 ){
		alert("<?php echo JText::_("YOUR_FORM_MUST_HAVE_A_TITLE"); ?>");
		enableTab('form');
		return false;		
	}
	...
	if(form.elements['params[thank]'].value.length == 0){
		alert("<?php echo JText::_("YOUR_FORM_MUST_SET_THE_THANK_YOU_URL"); ?>");
		enableTab('form');
		return false;
	}
...
function resizeWorkarea(){
	...
	var safetyMargin = 10;
	var clientArea  = $('design-box').getSize();
	...
	$('tab-toolbar').setStyle('height', (workareaArea.y - 52) +'px');
