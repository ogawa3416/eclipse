...
JLoader::register('JFormsElement', dirname(__FILE__) . '/parameter/element.php');
 ...
 */
class JFormsParameter extends JRegistry
{
	/**
	...
	public function setXML(&$xml)
	{
		...
		if (is_object($xml))
		{
			$attrubutes = $xml->attributes();
			if ($group = (string)$attrubutes['group'][0])
			{
				$this->_xml[$group] = $xml;
			}
			else
			...
			if ($dir = (string)$attrubutes['addpath'][0])
			{
				$this->addElementPath(JPATH_ROOT . str_replace('/', DIRECTORY_SEPARATOR, $dir));
			}
		}
	}
	...
	public function render($name = 'params', $group = '_default')
	{
		...
		$att = $this->_xml[$group]->attributes();
		if ($description = (string)$att['description'])
		{
...
//		foreach ($this->_xml[$group]->children() as $param)
	...
	public function getParam(&$node, $control_name = 'params', $group = '_default')
	{
		...
		$element = $this->loadElement((string)$att['type']);

		// Check for an error.
		if ($element === false)
		{
			$result = array();
			$result[0] = (string)$att['name'];
		...
		// Get value.
		$value = $this->get((string)$att['name'], (string)$att['default'], $group);
...
//			$xml = JFactory::getXML($path);
	...
	public function loadElement($type, $new = false)
	{
		...
		$elementClass = 'JFormsElement' . $type;
