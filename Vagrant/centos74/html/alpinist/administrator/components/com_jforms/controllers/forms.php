...
*
* @version		$Id: forms.php BsAlpinist 2.5.0 $
* @package		Joomla
 ...
 * @subpackage JForms.Controllers
 */
class FormsController extends JControllerLegacy
{
	var $_basePath ;
	/**
	...
	*/
	...
	function edit(){
		...
		$input = $app->input;
		$idArray = $input->get( 'cid', array(), 'array' );
		$id      = intval( $idArray[0] );
		...
		//Invalid form?
		if( $form == null ){
			$this->setRedirect('index.php?option=com_jforms', JText::_('FORM_NOT_FOUND'), 'error');
			return;
		}
		
		//Locked Form?
//		if ( JTable::isCheckedOut($user->get ('id'), $form->checked_out )){
		if ( $form->isCheckedOut($user->get ('id')) ){
			$msg = JText::sprintf(JText::_('DESCBEINGEDITTED'), JText::_('THE_ITEM'), $form->title);
	...
	function save()
	{
		...
		foreach( $pManager->settings['storage'] as $p ){
			$params['storagePluginParameters'][$p->name] = 
				$input->get( 'JFormSPlugin'.$p->name.'Parameters', array(), 'array');
		}
		...
		if( !$id ){
			$this->setRedirect('index.php?option=com_jforms', JText::_('FAILED_TO_SAVE_THE_FORM'), 'error');
		} else {
			$this->setRedirect('index.php?option=com_jforms', JText::_('FORM_SAVED'));
		}
	}
	...
	function publish()
	{
		...
		if ( !count($cids) ) {
			$this->setRedirect('index.php?option=com_jforms', JText::_('SELECT_AN_ITEM_TO_PUBLISH'), 'error');
		}

		$formModel = $this->getModel('form','JFormsModel');

		if( $formModel->publish($cids) ){
			$this->setRedirect('index.php?option=com_jforms', JText::_('PUBLISHED_SUCCESSFULY'));
		} else {
			$this->setRedirect('index.php?option=com_jforms', JText::_('FAILED_TO_PUBLISH_SELECTED_FORMS'), 'error');
		}
	}
	...
	function unpublish()
	{
		...
		JArrayHelper::toInteger($cids);
		if (!count($cids)) {
			$this->setRedirect('index.php?option=com_jforms', JText::_('SELECT_AN_ITEM_TO_UNPUBLISH'), 'error');
		}

		$formModel = $this->getModel('form','JFormsModel');

		if( $formModel->unpublish($cids) ){
			$this->setRedirect('index.php?option=com_jforms', JText::_('UNPUBLISHED_SUCCESSFULY'));
		} else {
			$this->setRedirect('index.php?option=com_jforms', JText::_('FAILED_TO_UNPUBLISH_SELECTED_FORMS'), 'error');
		}
	}
	 ...
	 */
	 function copy(){
		...
		$cids	  = $input->get( 'cid' , array(), 'array' );
		$newName  = $input->get( 'newName', null );

		JArrayHelper::toInteger($cids);
		
		if ( !count($cids) ) {
			$this->setRedirect('index.php?option=com_jforms', JText::_('SELECT_AN_ITEM_TO_COPY'), 'error');
		}
		...
		if( $formModel->copy($cids,$newName) ){
			$this->setRedirect('index.php?option=com_jforms', JText::_('Copied_successfuly'));
		} else {
			$this->setRedirect('index.php?option=com_jforms', JText::_('Failed_to_copy_form'), 'error');
		}
	}
	...
	function remove()
	{
		...
		JArrayHelper::toInteger($cids);
		if ( !count($cids) ) {
			$this->setRedirect('index.php?option=com_jforms', JText::_('SELECT_AN_ITEM_TO_DELETE'), 'error');
			return;
		}
		...
		$this->setRedirect('index.php?option=com_jforms',  JText::_('Forms_deleted'));
	}
	...
	function cancel()
	{
		...
		$this->setRedirect('index.php?option=com_jforms',  JText::_('ACTION_CANCELLED'), 'error');
	}
