...
/***  Modified for BsAlpinist  20110901 Start ***/ 
// Original line
...
// New line
				...
				$input = $app->input;
				$pjdir = "PJ".$input->get( 'jspjid' );
				$catid = $input->get( 'jscatid' );
				$pattern = array('/\$id/', '/\$username/', '/\$user(group|type)/', '/\$(group|profile)/', '/\$day/', '/\$month/', '/\$year/', '/\$pjid/', '/\$catid/');
				$replace = array($user->id, $user->username, $usertype, $profile->name, date('d'), date('m'), date('Y'), $pjdir,$catid);
/***  Modified for BlogStone  20100718 END ***/
