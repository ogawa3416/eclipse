...
/* 20160401 BugFix **/
				if($status=='inserted') {
					if( !array_key_exists('event_mid',$rows_data[$rid]) ) {
						$rows_data[$rid]['event_mid'] = 0;
					}
				}
...
/** blogstone **/
		$this->connector->event->trigger("afterafterProcessing",$action);
...
/*** blogstone Add **/
	private $oldevlength;//old event_elngth
	private $member;//old member
...
/*** blogstone Add **/
		...
		if( $status == 'updated') {
			$_db = JFactory::getDBO();
			$query = "SELECT event_length FROM #__bs_events_rec WHERE event_id=".$_db->Quote($id);
			$_db->setQuery( $query );
			$this->oldevlength = $_db->loadResult();
		}
	}
...
/*** blogstone Add **/
	function get_oldevlength(){
		return $this->oldevlength;
	}
}
?>