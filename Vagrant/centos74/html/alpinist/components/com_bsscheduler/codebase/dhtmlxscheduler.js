...
dtmlXMLLoaderObject.prototype.setXSLParamValue = function (B, C, D)
{
    ...
    var A = this.doXPath("/xsl:stylesheet/xsl:variable[@name='" + B + "']", D, "http://www.w3.org/1999/XSL/Transform", 
    "single");
    if (A != null) {
...
function dataProcessor(A)
{
    ...
    this.styles = 
    {
        updated : "font-weight:bold;", inserted : "font-weight:bold;", deleted : "text-decoration : line-through;", 
        invalid : "background-color:FFE0E0;", invalid_cell : "border-bottom:2px solid red;", error : "color:red;", 
...
dataProcessor.prototype = 
{
    ...
    _sendData : function (B, C)
    {
        ...
        var D = this.serverProcessor + (this._user ? (getUrlSymbol(this.serverProcessor) + ["dhx_user=" + this._user, 
...
/******* Blogstone *********/
	    var HH = this.obj._events[B];
	    if (HH) {
	    	if( HH.member ) {
				this.obj._events[B].member = HH.member.replace("#SEND#", "#NONE#");
			}
	    }
/******* Blogstone *********/
    },
    ...
    */
    loadUpdate : function ()
    {
        ...
        var C = this.serverProcessor + getUrlSymbol(this.serverProcessor) + ["dhx_user=" + this._user, 
        "dhx_version=" + A].join("&");
...
scheduler.init = function (C, A, B)
{
    ...
    dhtmlxEvent(window, "resize", function ()
    {
        ...
        // Add V3
		scheduler.set_finalformpos();
    });
    this.set_sizes();
    this.setCurrentView(A, B);
/** BsAlpinist **
	dhtmlxEvent(this._obj, "touchend", function (C)
    {
...
scheduler.xy = 
{
    nav_height : 30, min_event_height : 40, scale_width : 50, bar_height : 38, scroll_width : 4, scale_height : 20, 
    month_scale_height : 20, menu_width : 0, margin_top : 0, margin_left : 0, editor_width : 140, category_height : 50,
    month_day_height : 60
};
scheduler.keys = {
    edit_save : 13, edit_cancel : 27
};
scheduler.set_sizes = function ()
{
    ...
    this.set_xy(this._els.dhx_cal_navline_cal[0], B, this.xy.nav_height, 0, 0);
    this.set_xy(this._els.dhx_cal_navline[0], B, this.xy.nav_height, 0, this.xy.nav_height);
    this.set_xy(this._els.dhx_cal_header[0], B - E, this.xy.scale_height, A, this.xy.nav_height*2 + (this._quirks ?- 1 : 1));
    var F = this._els.dhx_cal_navline[0].offsetHeight;
    if (F > 0) {
        this.xy.nav_height = F
    }
    var C = this.xy.scale_height + this.xy.nav_height*2 + (this._quirks ?- 2 : 0);
    var CC = this.xy.scale_height + this.xy.nav_height*2 + this.xy.category_height + (this._quirks ?- 2 : 0);
    this.set_xy(this._els.dhx_cal_data[0], B, D - (CC + 2), 0, C + 2)

	var C = (C + 2) + D - (CC + 2);
    this.set_categoryxy(this._els.dhx_select_category[0], B, this.xy.category_height, 0, C);
};
...
scheduler.set_categoryxy = function (D, B, C, A, E)
{
    D.style.width = Math.max(0, B) + "px";
    D.style.height = Math.max(0, C) + "px";
    if (arguments.length > 3) {
...
{
    return {
        mode : this._mode, date : this._date, min_date : this._min_date, max_date : this._max_date, editor_id : this._edit_id, 
...
scheduler._click = 
{
    dhx_cal_data : function (C)
    {
        ...
        {
/* V3 ADD */
            var E = scheduler._locate_showbox(B);
            if(E == "1"){
                return false;
            }
...
/* BsAlpinist **/
            	...
            	scheduler._click.buttons.details(D);
            }
        }
        else {
...
//			if (ua.indexOf('iPhone') > 0 || ua.indexOf('iPod') > 0|| ua.indexOf('iPad') > 0 || (ua.indexOf('Android') > 0) && (ua.indexOf('Mobile') > 0) || ua.indexOf('Windows Phone') > 0) {
            if( ((navigator.pointerEnabled || navigator.msPointerEnabled) && navigator.maxTouchPoints > 0 )  || window.ontouchstart === null ) {
            	return scheduler._on_dbl_click(C || event)
			}
    ...
    {
        scheduler.setCurrentView(scheduler.date.add(scheduler.date[scheduler._mode + "_start"](scheduler._date), 
        (A || 1), scheduler._mode))
    },
    ...
    dhx_select_category : function (){
        var radioList = document.getElementsByName("select_category");
        for(var i=0; i<radioList.length; i++){
            if (radioList[i].checked) {
                scheduler.set_select_category(radioList[i].value);
                break;
            }
        }
...
scheduler._on_dbl_click = function (C, D)
{
    ...
    switch (A)
    {
        ...
        case "dhx_month_head":
        case "dhx_month_body":
        case "dhx_scale_half1":
        case "dhx_scale_half2":
            if (!scheduler.config.dblclick_create) {
                break
            }
            if(this._mode == 'year'){
            	F = this._mouse_correct_year(C);
            	if( !F ) return;
            } else {
...
scheduler._mouse_coords = function (D)
{
    ...
    else
    {
        F = 
        {
            x : D.clientX + (A.scrollLeft || E.scrollLeft || 0) - A.clientLeft, y : D.clientY + (A.scrollTop || E.scrollTop || 0) - A.clientTop
        }
    }
    F.x -= getAbsoluteLeft(this._obj) + (this._table_view ? 0 : this.xy.scale_width);
    F.y -= getAbsoluteTop(this._obj) + this.xy.nav_height*2 + (this._dy_shift || 0) + this.xy.scale_height - this._els.dhx_cal_data[0].scrollTop;
    F.ev = D;
...
scheduler._on_mouse_move = function (F)
{
    if (this._drag_mode)
    {
        ...
        if(this._drag_mode=='move' && !this._drag_pos ) {
        	this._drag_startpos = G;
        }
...
/** blogstone <<??????????**/
                ...
                this.addEvent(new Date(this._drag_start), new Date(E), this.locale.labels.new_event, this._drag_id, 
                G.fields);
...
/* Add Alpinist 20161218 >>>>*/
/*                    this.updateEvent(this._drag_id); */
					if( this._drag_mode == 'move' && (this._drag_startpos && this._drag_startpos.x == this._drag_pos.x && this._drag_startpos.y == this._drag_pos.y )) {
						;
					} else {
						this.updateEvent(this._drag_id);
					}
...
scheduler._on_mouse_down = function (A, B)
{
    ...
    if (this._drag_mode)
    {
        ...
        if (!this.config["drag_" + this._drag_mode] || !this.callEvent("onBeforeDrag", [C, this._drag_mode, 
        A])) {
...
/** blogstone >>**/
		    var myuserid = bssgetmyuserid();
		    if( this._drag_event ) {
		    	var drag = this._drag_event ;
				if(drag.createdby && drag.createdby != '0' && drag.member ) {
   			 		var snd = Array();
    				snd = drag.member.match(/^#[^#]*#/);
					var dstr = drag.member.replace(snd[0],"");
					if( (drag.createdby != myuserid && dstr.length > 0 ) || (drag.createdby != myuserid && drag.user != myuserid)) {
        	    		this._drag_mode = null;
						this._drag_id = null;
					}
				}
...
/** blogstone <<**/
    ...
    this._drag_startpos = null; /* Add Alpinist 20161218 */
    this._drag_start = null;
};
scheduler._on_mouse_up = function (B)
{
/* Add Alpinist 20161218 >>>>*/
	pos = this._mouse_coords(B);
	if( this._drag_mode == 'move' && (this._drag_startpos && this._drag_startpos.x == pos.x && this._drag_startpos.y == pos.y )) {
		this._drag_mode = null;
	}
/* Add Alpinist 20161218 <<<<*/
...
scheduler.setCurrentView = function (B, E)
{
	...
	if(jQuery('.dhx_cal_data').hasClass('calmonth')) jQuery('.dhx_cal_data').removeClass('calmonth');
    if(jQuery('.dhx_cal_data').hasClass('calweek')) jQuery('.dhx_cal_data').removeClass('calweek');
    if(jQuery('.dhx_cal_data').hasClass('calyear')) jQuery('.dhx_cal_data').removeClass('calyear');
    if(jQuery('.dhx_cal_data').hasClass('calagenda')) jQuery('.dhx_cal_data').removeClass('calagenda');
    jQuery('.dhx_cal_data').addClass('cal'+this._mode);
    this.set_holiday();
    this.set_finalformpos();
};
scheduler._render_x_header = function (B, D, E, C)
{

	daywk = this.getClassDayofweek(E,true)
...
/* BsAlpinist */
	if( this._mode == 'week' ) {
		A.innerHTML = A.innerHTML.substring(5,10);
	}
    C.appendChild(A)
};
scheduler._reset_scale = function ()
{
    ...
    for (var L = 0; L < E; L++)
    {
        ...
        if (!this._table_view)
        {
			daywk = this.getClassDayofweek(O,false);
            var F = document.createElement("DIV");
            var B = "dhx_scale_holder " + daywk;
            if (O.valueOf() == N.valueOf()) {
                B = "dhx_scale_holder_now"
            }
            F.className = B + " " + this.templates.week_date_class(O, N);
            this.set_xy(F, this._cols[L] - 1, P.hour_size_px * (P.last_hour - P.first_hour), C + this.xy.scale_width + 1, 
            0);
			F.id = this.templates.load_format(O);
		    var CC = new Date(1980, 1, 1, this.config.first_hour, 0, 0);
		    for (var D = this.config.first_hour * 1; D < this.config.last_hour; D++)
		    {
		        var FF = document.createElement("DIV");
		        FF.className = "dhx_scale_half1";
		        FF.style.height = Math.floor(this.config.hour_size_px / 2 ) - (this._quirks ? 0 : 1) + "px";
		        FF.style.width = (this._cols[L] - 1) + "px";
		        F.appendChild(FF);
		        var FF = document.createElement("DIV");
		        FF.className = "dhx_scale_half2";
		        FF.style.height = Math.floor(this.config.hour_size_px / 2 ) - (this._quirks ? 0 : 1) + "px";
		        FF.style.width = (this._cols[L] - 1) + "px";
		        F.appendChild(FF);
		        CC = this.date.add(CC, 1, "hour")
		    }
...
//    var L = (Math.floor(J.clientHeight / N) - 22);
	if( this._mode == 'month' ) {
		var L = scheduler.xy.month_day_height;
	} else {
		var L = 0;
	}
    ...
    for (var E = 0; E < N; E++)
    {
        ...
        for (var C = 0; C < 7; C++)
        {
			...
			daywk = this.getClassDayofweek(I)

			FF1 =" id=" + this.templates.load_format(I) + "_head";
			FF2 =" id=" + this.templates.load_format(I) + "_body";

//            F += "><div class='dhx_month_head" + daywk + "' >" + this.templates.month_day(I) + "</div><div class='dhx_month_body" + daywk + "' " + B[C] + "></div></td>";
            F += "><div class='dhx_month_head" + daywk + "'" + FF1 + " >" + this.templates.month_day(I) + "</div><div class='dhx_month_body" + daywk + "' " + B[C] + FF2 + "></div></td>";
...
scheduler.locale = 
{
    date : 
    {
        ...
        "October", "November", "December"], month_short : ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", 
        "Aug", "Sep", "Oct", "Nov", "Dec"], day_full : ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", 
        "Friday", "Saturday"], day_short : ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]
    },
    labels : 
    {
        dhx_cal_today_button : "Today", day_tab : "Day", week_tab : "Week", month_tab : "Month", new_event : "New event", 
        icon_save : "Save", icon_cancel : "Cancel", icon_details : "Details", icon_edit : "Edit", icon_delete : "Delete", 
        confirm_closing : "", confirm_deleting : "Event will be deleted permanently, are you sure?", section_description : "Description", 
        section_time : "Time period", full_day : "Full day", confirm_recurring : "Do you want to edit the whole set of repeated events?", 
        section_recurring : "Repeat event", button_recurring : "Disabled", button_recurring_open : "Enabled", 
        agenda_tab : "Agenda", date : "Date", description : "Description", year_tab : "Year",send_email:"E-mail to member", member_attention:"*)Only registrant can add or change" /** blogstone add send_email **/
   }
};
scheduler.config = 
{
    default_date : "%j %M %Y", month_date : "%F %Y", load_date : "%Y-%m-%d", week_date : "%l", day_date : "%D, %F %j", 
    hour_date : "%H:%i", month_day : "%d", xml_date : "%m/%d/%Y %H:%i", api_date : "%d-%m-%Y %H:%i", hour_size_px : 42, 
    time_step : 5, start_on_monday : 1, first_hour : 0, last_hour : 24, readonly : false, drag_resize : 1, 
    drag_move : 1, drag_create : 1, dblclick_create : 1, edit_on_create : 1, details_on_create : 0, click_form_details : 0, 
    server_utc : false, positive_closing : false, icons_edit : ["icon_save", "icon_cancel"], icons_select : ["icon_details", 
    "icon_edit", "icon_delete"], lightbox : 
    {
        sections : [ {
...
scheduler.init_templates = function ()
{
    ...
    {
        day_date : B(C.default_date), month_date : B(C.month_date),
        week_date : function (E, D)
        {
            return scheduler.templates.day_date(E) + " &ndash; " + scheduler.templates.day_date(scheduler.date.add(D, 
             - 1, "day"))
        },
        day_scale_date : B(C.day_date), month_scale_date : B(C.week_date), week_scale_date : B(C.day_date), 
        hour_scale : B(C.hour_date), time_picker : B(C.hour_date), event_date : B(C.hour_date), month_day : B(C.month_day), 
        xml_date : scheduler.date.str_to_date(C.xml_date, C.server_utc), load_format : B(C.load_date, 
        C.server_utc), xml_format : B(C.xml_date, C.server_utc), api_date : scheduler.date.str_to_date(C.api_date), 
...
scheduler.set_select_category = function (A)
{
	this._select_category = A;
}
scheduler.is_visible_events = function (A)
{
    if(this._select_category == '' || typeof this._select_category == 'undefined' || typeof A.category == 'undefined'){
        if (A.start_date < this._max_date && this._min_date < A.end_date) {
            return true
        }
    }
    else{
        if (A.start_date < this._max_date && this._min_date < A.end_date && A.category == this._select_category) {
            return true
        }
    }
    return false;
...
//    var B = Math.floor((M.clientWidth - D) / L._count);
	M_width = M.style.width;
	var B = Math.floor((parseInt(M_width) - D) / L._count);
    var E = L._sorder * B + 1;
    if (!L._inner) {
        B = B * (L._count - L._sorder)
    }
    var J = this._render_v_bar(L.id, D + E, K, B, O, L._text_style, scheduler.templates.event_header(L.start_date, 
    L.end_date, L), scheduler.templates.event_text(L.start_date, L.end_date, L));
...
/**
        ...
        var G = this._render_v_bar(L.id, E - D + 1, K, D, N.length * 20 + 26, "", "<div class='dhx_menu_head'></div>", 
        I, true);
        G.style.left = E - D + 1;
...
**/
/** blogstone >> **/
//		var pe = G.getElementsByTagName("*");
//		for(var i=0;i<pe.length;i++){
//			if( pe[i].className.indexOf('icon_delete') != -1 ) pe[i].style.display = "block";
//			else if( pe[i].className.indexOf('icon_edit') != -1 ) pe[i].style.display = "block";
//		}
//		var myuserid = bssgetmyuserid();
//		if(L.createdby && L.createdby != '0' && L.member  ) {
//    		var snd = Array();
//    		snd = L.member.match(/^#[^#]*#/);
//			var dstr = L.member.replace(snd[0],"");
//			if(( L.createdby != myuserid && dstr.length > 0  ) || (L.createdby != myuserid  && L.usery != myuserid )) {
//				for(var i=0;i<pe.length;i++){
//					if( pe[i].className.indexOf('icon_delete') != -1 ) pe[i].style.display = "none";
//					else if( pe[i].className.indexOf('icon_edit') != -1 ) pe[i].style.display = "none";
//				}
//			}
//		}
/** blogstone << **/
...
scheduler._render_v_bar = function (D, M, L, N, H, B, F, E, A)
{
    ...
    var showflg = '0';
	var cousor_set ='';
    if(scheduler._showbox_check(K) == true ){
	    E = scheduler.locale.labels.title_private_flg;
        showflg = '1';
        cousor_set = 'cursor:not-allowed;';
    }
//    var G = '<div event_id="' + D + '" class="' + I + '" style="position:absolute; top:' + L + "px; left:" + M + "px; width:" + (N - 4) + "px; height:" + H + "px;" + (B || "") + '">';
    var G = '<div event_id="' + D + '" show_flg="' + showflg + '" class="' + I + '" style="position:absolute; top:' + L + "px; left:" + M + "px; width:" + (N - 4) + "px; height:" + H + "px;" + (B || "") + '">';
    G += '<div class="dhx_header" style=" width:' + (N - 6) + 'px;" >&nbsp;</div>';
    G += '<div class="dhx_title">' + F + "</div>";
	EE='';
	if(F.indexOf('dhx_menu_head') == -1){
		EE = this._getPriority_icon(K);
	}
    G += '<div class="dhx_body" style=" width:' + (N - (this._quirks ? 4 : 14)) + "px; height:" + (H - (this._quirks ? 20 : 30)) + 'px;">' + EE + E + "</div>";
...
scheduler.render_event_bar = function (H)
{
    ...
    var showflg = '0';
	var cousor_set ='';
    if(scheduler._showbox_check(H) == true){
	    H.text = scheduler.locale.labels.title_private_flg;
        showflg = '1';
        cousor_set = 'cursor:not-allowed;';
    }
//    var C = '<div event_id="' + H.id + '" class="' + E + '" style="position:absolute; top:' + G + "px; left:" + I + "px; width:" + (A - I - 15) + "px;" + (H._text_style || "") + '">';
    var C = '<div event_id="' + H.id + '" show_flg="' + showflg + '" class="' + E + '" style="position:absolute; top:' + G + "px; left:" + I + "px; width:" + (A - I - 15) + "px; height:" + (D -11) + "px; " + (H._text_style || "") + cousor_set + '">';
	if (H._timed) {
        C += scheduler.templates.event_bar_date(H.start_date, H.end_date, H)
    }
    C += '<span style="display: block;">';
	if(showflg == '0'){
		C += this._getPriority_icon(H);
	}
...
/* V3 ADD */
scheduler._locate_showbox = function (A)
{
    var B = null;
    while (A && !B && A.getAttribute) {
        B = A.getAttribute("show_flg");
        A = A.parentNode
    }
    return B;
};
scheduler._showbox_check = function (A)
{
    var myuserid = bssgetmyuserid();

    if(typeof (A.createdby) == "undefined"){
        return false;
    }
    if(A.user != myuserid && A.createdby != myuserid && A.member.indexOf(myuserid + ',') == -1 && A.private_flg == '1'){
        return true;
    }
    return false;
}
scheduler._resize_container = function (A)
{
    box_pos = document.getElementById("scheduler_here");
//    light_pos = A.style.height;
	light_pos = jQuery('.dhx_cal_light').prop('scrollHeight');
		
    if(parseInt(box_pos.style.height) <= parseInt(light_pos)){
// Add V3
        box_pos.style.height = parseInt(light_pos) + 10 + "px";
    }
};
scheduler._getPriority_icon = function (A)
{
	    if(A.priority_kbn == 'priority_kbn_1'){
	        return '<i class="fa fa-exclamation-circle" style="color:red"></i>' + ' ';
	    }
	    else if(A.priority_kbn == 'priority_kbn_2'){
	        return  '<i class="fa fa-arrow-circle-down" style="color:blue"></i>' + ' ';
	    }
	    else if(A.priority_kbn == 'priority_kbn_3'){
	        return  '<i class="fa fa-question-circle" style="color:green"></i>' + ' ';
	    }
		else{
			return '';
		}
...
scheduler._magic_parser = function (I)
{
    ...
    for (var E = 0; E < A.length; E++)
    {
        ...
        for (var C = 0; C < K.length; C++) {
            G.push({
                key : K[C].getAttribute("value"), label : K[C].getAttribute("label")
            })
        }
    }
    if (A.length) {
...
scheduler.form_blocks = 
{
    ...
    select : 
    {
        render : function (D)
        {
            ...
            var C = "<div class='dhx_cal_ltext dhx_schplan' style='height:" + A + ";'><select style='width:442px;'>";
    ...
    time : 
    {
        render : function ()
        {
            ...
            C += "</select> ";
            return "<div style='height:26px; padding-top:0px; font-size:inherit;' class='dhx_cal_lsection dhx_section_time'>" + C + "<span style='font-weight:normal; font-size:10pt;'> &nbsp;&ndash;&nbsp; </span>" + C + "</div>";
        },
...
/** blogstone **/
    member : 
    {
        render : function (B)
        {
            var A = (B.height || "40") + "px";
            var C =  "<div class='dhx_cal_ltext' ><input type='hidden' id='sendemlst' value='' ><table id='membersblock' border='0' cellspacing='0' cellpadding='0' style='width:100%;height:100%;'><tr><td class='memberblk1' style='width:30%;height:" + A + ";'>";
//            C += "<select id='memberslist'  multiple='multiple' onchange='scheduler.touchmultifocus(\"Out\",this);'></select><div id='mbmemlist'></div></td>";
            C += "<div id='memberslist'></div><div id='mbmemlist'></div></td>";
			C += "<td class='memberblk2' style='width:10%;'>";
			C += "<div>&#60;&#60;</div><br />";
			C += "<div>&#62;&#62;</div></td>";
            C += "<td class='memberblk3' style='width:30%;'>";
			var dvact = document.getElementById("divcode");
            C += "<select id='bsdivcodeselect' onchange='scheduler.changedivcode();'>";
            var dlt = new Array();
            dlt = bssgetdivlist();
            for (var B = 0; B < dlt['code'].length; B++) {
            	var sled = '';
                if( dvact.value == dlt['code'][B] ) sled = "selected"; 
                C += "<option value='" + dlt['code'][B] + "' " + sled + " >" + dlt['name'][B] + "</option>";
            }
            C += "</select>";
            C += "<div id='uselectlist'>";
			var usct = document.getElementById("useridx");
            for (var B = 0; B < usct.options.length; B++) {
//                C += "<div value='" + usct.options[B].value + "' onclick='scheduler.touchmultifocus(\"In\"," + usct.options[B].value + ");'>" + usct.options[B].text + "</div>";
                C += "<div value=" + usct.options[B].value  + " onclick='scheduler.touchmultifocus(\"In\",this);'>" + usct.options[B].text + "</div>";
            }
			C+="</div>";
			C += "<td class='memberblk4' style='width:30%;'><div id='registrant'></div><br /><input type='checkbox' name='emsend' value='' onchange='scheduler.emsendchange();' disabled />"+scheduler.locale.labels.send_email+"<br /><br /><div id='member_attention' style='text-align:center;'>"+"</div>";
            C += "</td></tr></table></div>";
        ...
        set_value : function (B, C, A)
        {
            ...
            var reg = document.getElementById("registrant");
            var name = '';
            if( A.createdbyname ) name = A.createdbyname;
            else name = bssgetmyusername();
            reg.innerHTML = scheduler.locale.labels.registrant+ " : "+name;
            
			var myuserid = bssgetmyuserid();
            var eml = document.getElementsByName("emsend");
			eml[0].disabled=true;
            eml[0].checked = false;
			var memtbl = document.getElementById("membersblock");
			var pi = memtbl.getElementsByTagName("input");
			for(var i=0;i<pi.length;i++) pi[i].disabled=true;
			pi = memtbl.getElementsByTagName("select");
			for(var i=0;i<pi.length;i++) pi[i].disabled=true;
			var memmsg = document.getElementById("member_attention");
			memmsg.innerHTML = scheduler.locale.labels.member_attention;
            var eventid = {};
            var baseevent = true;
            if( A && A.id ) {
            	if( !isNaN(A.id) ) {
	            	...
	            	eventid = A.id.split('#');
	            	if( eventid.length > 1 ) baseevent = false;
            	}
            }
            if( baseevent && (!A.event_mid || A.event_mid == '0' || eventid[0] == A.event_mid) ) {
            	baseevent = true;
            } else {
                baseevent = false;
            }
			if( !A || !A.createdby || A.createdby == '0' || myuserid == A.createdby  ) {
				if( baseevent ) {
					var memtbl = document.getElementById("membersblock");
					var pi = memtbl.getElementsByTagName("input");
					for(var i=0;i<pi.length;i++) pi[i].disabled=false;
					pi = memtbl.getElementsByTagName("select");
					for(var i=0;i<pi.length;i++) pi[i].disabled=false;
				} else if( A.createdby && A.createdby == myuserid && eventid[0] == A.event_mid) {
					var memmsg = document.getElementById("member_attention");
					memmsg.innerHTML = scheduler.locale.labels.memowner_attention;
				}
				eml[0].disabled=false;
			}
            var mlt = document.getElementById("memberslist");
            if( !C ) {
	            B.firstChild.value = "#NONE#";
//	            while ( mlt.options.length>0 ) mlt.removeChild(mlt.options[0]);
                mlt.innerHTML='';
	            scheduler.mbmemlistchange();
            	return;
            }

            var snd = Array();
            snd = C.match(/^#[^#]*#/);
	        var dstr = C.replace(snd[0],"");
	        var imems = Array();
	        imems = dstr.split("/");
/*
            while ( mlt.options.length>0 ) mlt.removeChild(mlt.options[0]);
...
*/
			...
			imem = imems[i].split(",");
                if( 1 in imem ) {
                    mlt.innerHTML = mlt.innerHTML + "<div value='" + imem[0] + "' onclick='scheduler.touchmultifocus(\"Out\",this);'>" + imem[1] + "</div>";
			    }
			}
			scheduler.mbmemlistchange();
        },
    ...
    private_flg : 
    {
        render : function (B)
        {
            var A = (B.height || "16") + "px";
            return "<div class='dhx_cal_ltext' style='height:" + A + ";'><input type='checkbox' id='private_flg' name='private_flg' /></div>";
        },
        set_value : function (B, C, A)
        {
		var obj = document.getElementById("private_flg");
		if(C == '1'){
			//checked
			obj.checked = true;
		}
        ...
        get_value : function (B, A)
        {
		if(document.getElementById("private_flg").checked == true){
			return '1';
		}
		else{
			return '0';
...
***/
          A.style.top = "50px";
          A.style.left = Math.round((this._obj.offsetWidth - A.offsetWidth) / 2 ) + "px";
    }
};
...
/* V3 add */
    scheduler._resize_container(A);
    document.getElementById("dhx_select_category").style.display ="none";
    document.getElementById("dhx_cal_tooltips").style.display ="none";

/* Add V3 */
	this.boxopenpos = jQuery("body").scrollTop();
	scpos = jQuery(".dhx_cal_cover").offset().top-60;
	jQuery("html,body").animate({scrollTop : scpos}, {queue : false});
};
scheduler._fill_lightbox = function (H, E)
{
    ...
    else
    {
        ...
        B[2].innerHTML = (this.templates.event_bar_text(D.start_date, D.end_date, D) || "").substr(0, 
        70)
    }
/** blogstone >>**/
	document.getElementById("savebtn").style.display = "block";
	document.getElementById("deletebtn").style.display = "block";
	var pe = this._lightbox.getElementsByTagName("input");
	for(var i=0;i<pe.length;i++){
		pe[i].disabled=false;
	}
	pe = this._lightbox.getElementsByTagName("textarea");
	for(var i=0;i<pe.length;i++){
		pe[i].disabled=false;
	}
	pe = this._lightbox.getElementsByTagName("select");
	for(var i=0;i<pe.length;i++){
		pe[i].disabled=false;
	}
/** blogstone <<**/
...
/** blogstone >>**/
	var myuserid = bssgetmyuserid();
	if(D.createdby && D.createdby != '0' && D.user != '0' && D.member) {
    	var snd = Array();
    	snd = D.member.match(/^#[^#]*#/);
		var dstr = D.member.replace(snd[0],"");
		if( (D.createdby != myuserid  && dstr.length > 0 ) || (D.createdby != myuserid  && D.user != myuserid )) {
...
/** blogstone add >> **/
	var ev = this.getEvent(D.id);
	if( !D['createdby'] )  D['createdby'] = ev.createdby ? ev.createdby : bssgetmyuserid(); 
	if( !D['createdbyname'] ) D['createdbyname'] = ev.createdbyname ? ev.createdbyname : bssgetmyusername();
	if( !D['event_mid'] ) {
		if( ev.event_mid ) D['event_mid'] = ev.event_mid;
		else {
			if( ev.event_pid ) {
				var oldev = this.getEvent(ev.event_pid);
				D['event_mid'] = oldev.event_mid ? oldev.event_mid : '0';
			}
		}
	}
/** blogstone add << **/
...
/* BsAlpinist */
	...
	for( var i = 0, il = elements.length; i < il; i++ ) {
		element = elements[i];
		if( /\bdhx_cal_data\b/.test( element.className ) ) break;
	}
	element.style.visibility = 'visible';
/* V3 ADD */
    box_pos = document.getElementById("scheduler_here");
    box_pos.style.height = this._box_pos_height;
    document.getElementById("dhx_cal_tooltips").style.display ="block";
    scheduler.set_finalformpos();
    if(this.boxopenpos) {
		jQuery("body").scrollTop(this.boxopenpos);
    }

};
scheduler.hideCover = function (A)
{
	jQuery('.dhx_cal_cover').unbind();
    if (A) {
...
scheduler.hide_cover = function ()
{
    if (this._cover) {
		...
		var elmt = '';
		for( var i = 0, il = elmts.length; i < il; i++ ) {
			elmt = elmts[i];
			if( /\bdhx_cal_cover\b/.test( elmt.className ) ){
				var elmt_parent=elmt.parentNode;
				elmt_parent.removeChild(elmt);
				break;
			}
		}
    }
...
***/
	box_pos = document.getElementById("scheduler_here");
	box_pos.insertBefore(this._cover,this._lightbox);
// Add V3
	jQuery('.dhx_cal_cover').click(function(){
...
scheduler.save_lightbox = function ()
{
    ...
    this.hide_lightbox()
    document.getElementById("dhx_select_category").style.display ="";
};
...
scheduler.endLightbox = function (B, A)
{
    ...
    this.hideCover(A);
/* BsAlpinist */
	jQuery('.dhx_cal_data').css('visibility','visible');
};
...
/* V3 ADD */
                        scheduler._resize_container(E.parentNode.parentNode.parentNode.parentNode);
                    }
            }
        }
    };
...
scheduler.setLightboxSize = function ()
{
    ...
    B.style.height = A.scrollHeight + 62 + "px";
    A.style.height = A.scrollHeight + 0 + "px";
};
...
/* Add V3 */
		tmparry = E.concat();
		E[2] = tmparry[4]; // category
		E[3] = tmparry[2]; // eventdv
		E[4] = tmparry[3]; // priority
        for (var B = 0; B < E.length; B++)
        {
            var F = this.form_blocks[E[B].type];
            if (!F) {
            ...
            C += "<div class='subblock_"+E[B].name+"'><div id='" + E[B].id + "' class='dhx_cal_lsection'>" + A + this.locale.labels["section_" + E[B].name] + "</div>" + F.render.call(this, 
            E[B])+'</div>';
        }
        ...
        this._box_pos_height = box_pos.style.height;
        
        jQuery('.subblock_category select').change(function(){
			scheduler.categorychange();
		});
    }
    return this._lightbox;
};
/** blogstone  Add id -> dhx_btn_set **/
scheduler._lightbox_template = "<div class='dhx_cal_ltitle'><span class='dhx_mark'>&nbsp;</span><span class='dhx_time'></span><span class='dhx_title'></span></div><div class='dhx_cal_larea'></div><div class='dhx_btn_set' id='savebtn'><div class='dhx_save_btn'></div><div>&nbsp;</div></div><div class='dhx_btn_set'><div class='dhx_cancel_btn'></div><div>&nbsp;</div></div><div class='dhx_btn_set'  id='deletebtn' style='float:right;'><div class='dhx_delete_btn'></div><div>&nbsp;</div></div>";
...
scheduler.attachEvent("onLightBox", function ()
{
    ...
    /* BsAlpinist */
    scheduler.resetuselectlist();
    scheduler.categorychange();
    jQuery('.dhx_cal_data').css('visibility','hidden');
});
scheduler.form_blocks.select.set_value = function (B, C, A)
{
...
scheduler.render_footer_category = function ()
{
    var F = this.config.lightbox.sections;

    for (var C = 0; C < F.length; C++)
    {
        if (F[C].map_to == "category") {
            var B = F[C].options;
			var D = "name='select_category' ";
            var C = "<p>";
                C += "<input type='radio' " + D + " id='category_all' value='' checked='checked'><label for='category_all'>" + scheduler.locale.labels.category_all + "&nbsp;</label>"
            for (var A = 0; A < B.length; A++) {
				if(B[A].key == "category_0"){
					continue;
				}
                C += "<input type='radio' " + D + "  id='" + B[A].key + "' value='" + B[A].key + "'><label for='" + B[A].key + "'>" + B[A].label + "&nbsp;</label>"
            }
		    C += "</p>";

			document.getElementById("dhx_select_category").innerHTML = C;
			return;
        }
    }
};
scheduler.getClassDayofweek= function (A, B)
{
	var daywk='';
	if(A.getDay() == 6){
	    daywk = " day_sat";
...
scheduler.set_finalformpos = function ()
{
	datah = 586;
    if( this._mode == 'month' ) {
    	jQuery('.dhx_cal_data').css('height','auto');
    	datah = jQuery('.dhx_cal_data').eq(0).prop('scrollHeight');
    } else if( this._mode == 'week' ) {
    	h = jQuery('.dhx_cal_data').eq(0).outerHeight(true);
    	jQuery('.dhx_cal_data').eq(0).height(h);
    	datah = jQuery('.dhx_cal_data').eq(0).prop('scrollHeight');
    } else if( this._mode == 'year' ) {
    	jQuery('.dhx_cal_data').css('height','auto');
    	h = jQuery('.dhx_cal_data').eq(0).prop('scrollHeight');
    	jQuery('.dhx_cal_data').eq(0).height(h);
    	datah = jQuery('.dhx_cal_data').eq(0).prop('scrollHeight');
    } else {
    	jQuery('.dhx_cal_data').eq(0).height(datah);
    	datah = jQuery('.dhx_cal_data').eq(0).prop('scrollHeight');
    }
	hereh = jQuery('.dhx_cal_navline_cal').eq(0).outerHeight(true);
	hereh += jQuery('.dhx_cal_navline').eq(0).outerHeight(true);
	hereh += jQuery('.dhx_cal_header').eq(0).outerHeight(true);
	hereh += datah
	hereh += jQuery('#dhx_select_category').eq(0).outerHeight(true)+16;
    jQuery('#scheduler_here').height(hereh);
    
    cath = jQuery('#dhx_select_category').outerHeight(true);
    jQuery('#dhx_select_category').css('top',(hereh-cath)+'px');
};
/* setholidays */
scheduler.set_holiday = function ()
{
	tmpdate = this.date[this._mode + "_start"](new Date(this._date.valueOf()));
	stdate = this.templates.load_format(tmpdate);
	seturllink = getHolidayslink(stdate,this._mode);
	mode = this._mode;
    dhtmlxAjax.get(seturllink, function (getdata)
    {
        this.holidays = getdata.xmlDoc.responseText;
		this.holidays = this.holidays.replace(/\[|\]/g,'');
		this.holidays = this.holidays.replace(/\"/g,'');
		this.holidays = this.holidays.split(',');
		for(var cnt =0 ; cnt < this.holidays.length ; cnt++){
			if(mode == "month" || mode == "year"){
				checkid = document.getElementById(this.holidays[cnt] + "_head");
				if( checkid != null){
					checkid.classList.add('day_holiday');
					checkid = document.getElementById(this.holidays[cnt] + "_body");
					checkid.classList.add('day_holiday');
				}
			}
			else{
				checkid = document.getElementById(this.holidays[cnt])
				if( checkid != null){
					checkid.classList.add('day_holiday');
				}
			}
...
scheduler._mouse_correct_year = function (D)
{
	if( !D.target.id || (D.target.id.indexOf('_head') == -1 && D.target.id.indexOf('_body') == -1 ) ) return false;
	if( D.target.parentElement.className.indexOf('dhx_after') != -1 || D.target.parentElement.className.indexOf('dhx_before') != -1 ) return false;
		
	var tgst = D.target.id.replace(/_head|_body/g , '' ) ;
	offs = this.config.first_hour*3600*1000;
	tmz = (new Date()).getTimezoneOffset()*60*1000;
	var tgdt = new Date(tgst);
	var tgtm = tgdt.getTime();
	tgtm= tgtm+tmz+offs;
    return tgtm;
};