...
<h<?php echo $headerLevel; ?> class="js_heading"><span class="backh"> <span
	...
	title="<?php echo JText::_('TPL_BSPORTAL3_CLICK'); ?>"
	onclick="auf('module_<?php echo $module->id; ?>'); return false"
	class="opencloselink" id="link_<?php echo $module->id?>"> <span
	class="no"><img src="templates/alpinist-green/images/plus.png"
	alt="<?php if ($state == 1) { echo JText::_('TPL_BSPORTAL3_ALTOPEN');} else {echo JText::_('TPL_BSPORTAL3_ALTCLOSE');} ?>" />
...
function modChrome_beezTabs($module, $params, $attribs)
{
	...
	if ($modulecount == 1) {
		...
		foreach($modules as $rendermodule) {
			...
			{
			echo '<a href="#" class="unseen" onclick="nexttab(\'module_'. $rendermodule->id.'\');return false;" id="next_'.$rendermodule->id.'">'.JText::_('TPL_BSPORTAL3_NEXTTAB').'</a>';
			}
			echo '</div>';
		}
		$modulecount--;
