...
window.addEvent('domready', function () {
	...
	if (document.id('user6')) {
		document.id('user6').setProperties( {
			role : 'contentinfo'
		});
	}
...
function auf(key) {
	...
	if (el.style.display == 'none') {
		...
		if (key == 'user6') {
			document.id('user6').setStyle('display', 'block');
		...
		if (key != 'user6') {
			...
			elternbild.focus();
			document.id('user6').setStyle('width', '0');
		}

		if (key == 'user6') {
			document.id('user6').setStyle('display', 'none');
			grafik = document.id('bild');
			grafik.innerHTML = rightopen;
			grafik.focus();
		}
